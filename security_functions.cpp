...
cbc_decrypt(unsigned char *ciphertext, int ciphertext_len, unsigned char *key, unsigned char *plaintext){
	int ret;
	int plaintext_len;
	const EVP_CIPHER* cipher = EVP_aes_128_cbc();
	int iv_len = EVP_CIPHER_iv_length(cipher);
	...
	plaintext_len=iv_len;
	int update_len=0;
	//for blocco
	ret=EVP_DecryptUpdate(ctx, plaintext+plaintext_len, &update_len, ciphertext, ciphertext_len);
